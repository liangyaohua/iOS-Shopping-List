...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event;
{
    ...
    if ((self.failed = [self shouldFail]))
        return;
//    NSLog(@"Content offset y: %f, triggerview.frame: %f, bounds: %f, inset %f", self.scrollView.contentOffset.y, CGRectGetMinY(self.triggerView.frame), self.scrollView.bounds.origin.y, self.scrollView.contentInset.top);
    
    if ((self.scrollView.contentOffset.y + self.scrollView.contentInset.top) <= CGRectGetMinY(self.triggerView.frame))
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView;
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:MOONotificationContentOffsetChanged object:self userInfo:[NSDictionary dictionaryWithObject:[NSValue valueWithCGPoint:offset] forKey:MOOKeyContentOffset]];
}
